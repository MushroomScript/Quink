...
| **#11 提醒分家** | 个人提醒 (todoDue, 每人自管, 删了内容也响) + 群提醒新表 (群管理员设, 群所有人收) + 群组提醒接收开关 (每人自己控该群提醒) | 11a ✅ done (后端), 11b pending (前端) | ~500 行 (估) |
...
**拆分**: 10a 后端地基 (schema + 6 endpoint + createNotification helper + cleanup 30 天) ✅ done / 10b 前端 view (Notifications.vue + 路由 + Sidebar 入口 + SSE notification-new/changed handler + store + 4 tab + 弹窗确认) ✅ done / 10c 接入点改造 (notes 5 处 edit-request/approve/reject/comment-added/duplicated + groups 7 处 join-request/joined×2/rejected/removed/promoted/demoted/dissolved + reminder-due) ✅ done.

**留待后续 PR**:
- `note-deleted-by-admin` 通知 → 进 PR #12 (群组回收站 + 审计) 一起做, 因为依赖 admin 删笔记的完整流程
- `fork-by-other` 通知 → 进 PR #13 (收尾) 一起做, 跟 fork UI/AI/统计 一同收尾
