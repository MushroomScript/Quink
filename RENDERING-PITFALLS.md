...
- **Phase 1 改造 (2026-05-30): "显示比例" 切到 Electron `webContents.setZoomFactor()` 模式**, 替代老 `prefs.fontSize` 改 `<html>` font-size 的 rem-based 缩放。**Electron 桌面端**主进程监听 `sync-zoom` IPC 后给 3 个窗口(main / Capture / AiChat)各自 `setZoomFactor(level/100)`, paint 层等比缩放所有元素(px/rem/transform 同步), **永久消除上一条提到的 round 奇偶对齐问题**。Web/PWA 端 Phase 2 用 CSS `zoom` 属性补齐(暂保留默认 100%)。Phase 3 待清理: 所有为 rem-based 字号补偿做的 workaround (`ToggleSwitch.vue` 的 `translate-y-px` / `Settings.vue` 6 处保存按钮的 `pt-[0.32rem] pb-[0.43rem]` / `Sidebar.vue` 徽章的 `leading-[1.125rem]` / d58382d 系列改动 等) 都可以回归普通写法。**新场景优先用 zoom 不用 rem 补偿**。字段映射: 老 `prefs.fontSize` (12-22 px) → 新 `prefs.zoomLevel` (75-200 整数百分比, 8 档 75/80/90/100/110/125/150/200), DB 启动迁移 (`packages/server/src/db/index.ts`) + 前端 `buildPrefs()` 清老字段。IPC 名 `sync-font-size` → `sync-zoom`, `font-size-changed` 删除。localStorage `quink_font_size` → `quink_zoom_level`。
