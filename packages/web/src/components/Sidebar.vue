      ...
      <Transition enter-active-class="transition duration-100 ease-out" enter-from-class="opacity-0 scale-95"
        ...
        <div v-if="showUserMenu"
          ...
          <button @click="openNotifications"
            ...
            <!-- badge 样式复用待办那套 (蘑菇 2026-06-08 之前精调过 padding/字号/颜色). translate-y-[1.5px] 让 badge 含字整体下移 1.5px 跟 PhBell icon baseline 对齐 -->
            <span v-if="notificationsStore.unread.total > 0"
              class="ml-auto translate-y-[0.25px] inline-flex items-center justify-center min-w-[18px] h-[18px] pb-[1px] bg-red-400/70 text-white text-[11px] leading-none font-semibold tabular-nums rounded-full"
