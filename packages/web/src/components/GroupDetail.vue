...
// 沿用项目现有模式 (Inspiration/Notes/Todos), 只初始化读一次, 不监听 resize.
const isMobile = ref(window.innerWidth < 768);
// 群成员默认折叠 (仅移动端), 点标题行展开/收起
...
<template>
  <div class="h-full overflow-y-auto overflow-x-hidden px-4 md:px-8 py-6">
    <div class="max-w-6xl mx-auto w-full">
      ...
      <div v-else-if="detail" class="space-y-5">
        ...
        <!-- 主体两栏: 桌面 笔记(order-1)在左 + 公告成员(order-2)靠右侧栏 (改动前布局); 移动端堆叠 公告成员在上、笔记在下 -->
        <div class="flex flex-col md:flex-row gap-5 items-start">
          <!-- 公告 + 成员: 移动全宽在上, 桌面 13rem 右侧栏 -->
          <aside class="w-full md:w-52 shrink-0 space-y-5 md:order-2">
            <!-- 群公告 (两端一致) -->
            ...
            <!-- 群组成员: 移动端折叠 (头像叠放预览, 点标题展开全部) -->
            <section v-if="isMobile" class="bg-gray-50 rounded-xl p-4">
                ...
                <div class="flex items-center gap-1.5 min-w-0">
                  ...
                  <div v-if="!membersExpanded" class="flex -space-x-2 shrink-0">
                    <template v-for="m in sortedMembers.slice(0, 5)" :key="m.userId">
                      ...
                      <div v-else class="w-6 h-6 rounded-full bg-primary-light text-primary-dark flex items-center justify-center text-[10px] font-bold ring-2 ring-gray-50">
          ...
          <!-- 群内笔记: 移动 masonry 多列, 桌面单列 (改动前) -->
          <section class="w-full md:flex-1 min-w-0 md:order-1">
            ...
            <!-- 移动端: masonry 多列 -->
            <div v-else-if="isMobile" ref="masonryRoot" class="notes-masonry">
